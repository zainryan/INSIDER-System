...
inline struct fpga_pci_bar *fpga_pci_bar_get(pci_bar_handle_t handle) {
  return &bars[handle];
}

static int fpga_pci_bar_set_mem_base_size(pci_bar_handle_t handle,
...
static inline void *fpga_pci_bar_get_mem_at_offset(pci_bar_handle_t handle,
                                                   uint64_t offset,
                                                   uint64_t xaction_size) {
  log_debug("handle=%d", handle);
...
static int fpga_pci_bar_alloc(void) {
  log_debug("enter");
...
static int fpga_pci_bar_free(pci_bar_handle_t handle) {
  log_debug("handle=%d", handle);
...
static int fpga_pci_check_file_id(char *path, uint16_t id) {
  ...
  fail_on(tmp_id != id, err_close, "path=%s, id(0x%04x) != expected id(0x%04x)",
          path, tmp_id, id);
...
static int fpga_pci_bar_attach(struct fpga_slot_spec *spec, int pf_id,
                               int bar_id, bool write_combining, int *handle) {
  int fd = -1;
  log_debug("enter");
  ...
  log_info("vendor_id=0x%04x, device_id=0x%04x, DBDF:" PCI_DEV_FMT
           ", resource_num=%u, size=%" PRIu64 ", burstable=%u",
           map->vendor_id, map->device_id, map->domain, map->bus, map->dev,
           map->func, bar_id, map->resource_size[bar_id],
  ...
  *handle = -1;
  ...
  int ret = snprintf(sysfs_name, sizeof(sysfs_name),
                     "/sys/bus/pci/devices/" PCI_DEV_FMT "/vendor", map->domain,
                     map->bus, map->dev, map->func);

  fail_on(ret < 0, err, "Error building the sysfs path for vendor");
  fail_on((size_t)ret >= sizeof(sysfs_name), err,
          "sysfs path too long for vendor");
  ...
  ret = snprintf(sysfs_name, sizeof(sysfs_name),
                 "/sys/bus/pci/devices/" PCI_DEV_FMT "/device", map->domain,
                 map->bus, map->dev, map->func);
  ...
  snprintf(sysfs_name, sizeof(sysfs_name),
           "/sys/bus/pci/devices/" PCI_DEV_FMT "/resource%u%s", map->domain,
           map->bus, map->dev, map->func, bar_id, wc_suffix);
...
int fpga_pci_attach(int slot_id, int pf_id, int bar_id, uint32_t flags,
  ...
  fail_on(bar_id < 0 || bar_id >= FPGA_BAR_PER_PF_MAX, err, "Invalid bar_id=%d",
          bar_id);
  ...
  if (flags & BURST_CAPABLE) {
    ...
    fail_on(ret, err,
            "bar is not BURST_CAPABLE (does not support write "
            "combining.)");
...
int fpga_pci_poke(pci_bar_handle_t handle, uint64_t offset, uint32_t value) {
  log_debug("handle=%d, offset=0x%" PRIx64 ", value=0x%08x", handle, offset,
            value);

  uint32_t *reg_ptr = (uint32_t *)fpga_pci_bar_get_mem_at_offset(
...
int fpga_pci_poke64(pci_bar_handle_t handle, uint64_t offset, uint64_t value) {
  log_debug("handle=%d, offset=0x%" PRIx64 ", value=0x%" PRIx64, handle, offset,
            value);

  uint64_t *reg_ptr = (uint64_t *)fpga_pci_bar_get_mem_at_offset(
...
int fpga_pci_write_burst(pci_bar_handle_t handle, uint64_t offset,
  ...
  uint32_t *reg_ptr = (uint32_t *)fpga_pci_bar_get_mem_at_offset(
      handle, offset, sizeof(uint32_t) * dword_len);
  fail_on(!reg_ptr, err, "fpga_plat_get_mem_at_offset failed");
...
int fpga_pci_get_address(pci_bar_handle_t handle, uint64_t offset, size_t len,
                         void **ptr) {
