...
void dram_write_throttle_unit(
  ...
  bool has_write_req_apply = false;
...
#pragma HLS pipeline
    ...
    if (has_write_req_apply ||
        (has_write_req_apply =
             before_throttle_unified_dram_write_req_apply.read_nb(
                 write_req_apply))) {
      if (after_throttle_unified_dram_write_req_apply.write_nb(
              write_req_apply)) {
        has_write_req_apply = false;
      }
    }
