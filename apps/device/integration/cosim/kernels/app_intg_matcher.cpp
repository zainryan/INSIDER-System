...
#include "constant.h"
#include "structure.h"
void app_intg_matcher(ST_Queue<bool> &reset_app_intg_matcher,
...
#pragma HLS array_partition variable = query complete
  ...
  while (1) {
    bool dummy;
    if (reset || (reset = reset_app_intg_matcher.read_nb(dummy))) {
      valid_param = false;
      get_thresParam = false;
      readParamCnt = 0;
      query_thres = 0;
      record_thres = 0;
      unsigned int dummy0;
      app_intg_input_params.read_nb(dummy0);
      struct APP_Data dummy1;
      app_intg_input_data.read_nb(dummy1);
      reset_cnt++;
