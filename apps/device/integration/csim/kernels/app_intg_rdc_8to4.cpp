...
void app_intg_rdc_8to4(ST_Queue<bool> &reset_app_intg_rdc_8to4,
...
#pragma HLS pipeline
    bool dummy;
    if (reset || (reset = reset_app_intg_rdc_8to4.read_nb(dummy))) {
      struct APP_Reduce_Record8 dummy0;
      app_intg_rdc_input_8to4.read_nb(dummy0);
      reset_cnt++;
