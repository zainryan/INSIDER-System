...
#pragma HLS array_partition variable = rows_buffer complete dim = 2
  _Bool finish_reading_stencil_points = false;
  ;
  ;
...
#pragma HLS array_partition variable = cols_buffer complete dim = 2
  ...
  ;
  ;
  struct Point prev_points_input_data[16];
  ;
  ;
...
#pragma HLS array_partition variable = prev_points_input_data complete dim = 0
  ...
  while (1) {
    bool dummy;
    if (reset || (reset = reset_app_stencil.read_nb(dummy))) {
      for (int i0 = 0; i0 < 3; i0++) {
        for (int i1 = 0; i1 < 3; i1++) {
          stencil[i0][i1] = Point();
        }
      }
      finish_reading_stencil_points = false;
      app_input_read_times = 0;
      num_line = 0;
      for (int i0 = 0; i0 < 16; i0++) {
        prev_points_input_data[i0] = Point();
      }
      stencil_height_idx = 0;
      stencil_width_idx = 0;
      unsigned int dummy0;
...
#pragma HLS pipeline
      if (!finish_reading_stencil_points) {
        ...
        if (app_input_params.read_nb(input_param)) {
          stencil[stencil_height_idx][stencil_width_idx] =
              uint_to_point(input_param);
          stencil_width_idx++;
