...
void write_mode_dram_helper_app(
  ...
  // in byte unit
  ap_uint<40> extents_start_arr[MAX_EXTENT_NUM];
#pragma HLS array_partition variable = extents_start_arr complete
  ap_uint<40> extents_len_arr[MAX_EXTENT_NUM];
#pragma HLS array_partition variable = extents_len_arr complete
  ...
  ap_uint<40> cur_extents_offset = 0;
  ap_uint<40> cur_extents_len;
  ap_uint<40> cur_extents_start;
  ap_uint<40> cur_extents_left_space;
    ...
    if (reset || (reset = reset_dram_helper_app.read_nb(dummy))) {
      ...
      if (state == 0) {
        ...
        if (cur_extents_left_space >
          ...
          write_req_apply.addr = cur_extents_offset + cur_extents_start;
          ...
          Dram_Write_Req_Apply write_req_apply;
          write_req_apply.num = (cur_extents_left_space >> DATA_BUS_WIDTH_LOG2);
          write_req_apply.addr = cur_extents_offset + cur_extents_start;
          if (device_dram_write_req_apply.write_nb(write_req_apply)) {
            cached_dram_write_req_apply.num -=
                (cur_extents_left_space >> DATA_BUS_WIDTH_LOG2);
