...
int main() {
  ...
  ST_Queue<bool> reset_app_stencil(4);
  ...
  std::thread t33(
      ...
      std::ref(cached_dramD_write_req_data));
  std::thread t34(
      reset_propaganda, std::ref(reset_app_stencil), std::ref(reset_sigs),
      std::ref(reset_read_mode_dram_helper_app),
      std::ref(reset_write_mode_dram_helper_app),
      std::ref(reset_read_mode_pcie_helper_app),
      std::ref(reset_write_mode_pcie_helper_app),
      ...
      std::ref(reset_app_input_data_merger),
      std::ref(reset_write_mode_pre_merged_app_input_data_forwarder));
  std::thread t35(app_stencil, std::ref(reset_app_stencil),
...
void user_simulation_function() {
  ...
  std::cout << "Expected total read bytes = " << sizeof(real_output_pictures)
            << std::endl;
  ...
  for (int i = 0; i < PICTURE_HEIGHT; i++) {
    for (int j = 0; j < PICTURE_WIDTH; j++) {
      ...
      if (host_point_to_uint(expected) != host_point_to_uint(real)) {
        std::cout << "Failed at i = " << i << ", j = " << j << std::endl;
        std::cout << "Expected (r, g, b, a) = " << (int)expected.red << " "
                  << (int)expected.green << " " << (int)expected.blue << " "
                  << (int)expected.alpha << std::endl;
        std::cout << "Real (r, g, b, a) = " << (int)real.red << " "
                  << (int)real.green << " " << (int)real.blue << " "
                  << (int)real.alpha << std::endl;
        exit(-1);
      }
    }
  }
