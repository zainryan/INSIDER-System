...
int main() {
  ...
  ST_Queue<bool> reset_app_rle_combine(4);
  ST_Queue<bool> reset_app_rle_expand(4);
  ST_Queue<bool> reset_app_rle_prefix_sum(4);
  ...
  std::thread t33(
      ...
      std::ref(cached_dramD_write_req_data));
  std::thread t34(
      reset_propaganda, std::ref(reset_app_rle_combine),
      std::ref(reset_app_rle_expand), std::ref(reset_app_rle_prefix_sum),
      ...
      std::ref(reset_app_input_data_merger),
      std::ref(reset_write_mode_pre_merged_app_input_data_forwarder));
  std::thread t35(app_rle_prefix_sum, std::ref(reset_app_rle_prefix_sum),
                  std::ref(app_input_params), std::ref(app_input_data),
                  std::ref(app_rle_expand_input_data));
  std::thread t36(app_rle_expand, std::ref(reset_app_rle_expand),
                  std::ref(app_rle_expand_input_data),
                  std::ref(app_rle_combine_input_data));
  std::thread t37(app_rle_combine, std::ref(reset_app_rle_combine),
                  std::ref(app_rle_combine_input_data),
                  std::ref(app_output_data));
  std::thread t38(host_pcie_simulator);
  std::thread t39(fpga_dramA_simulator);
  std::thread t40(fpga_dramB_simulator);
  std::thread t41(fpga_dramC_simulator);
  std::thread t42(fpga_dramD_simulator);
...
#define OUTPUT_SIZE (64 * 30)

unsigned char input_data[OUTPUT_SIZE];
unsigned char expected_result[OUTPUT_SIZE];
unsigned char real_file_buf[OUTPUT_SIZE];

unsigned long long extents_start_idx[] = {0, 8192, 40960};
unsigned long long extents_len[] = {64 * 10, 64 * 10, 64 * 10};
...
void gen_data() {
  ...
  unsigned char *data_ptr = input_data;
  while (output_size < OUTPUT_SIZE) {
    unsigned char number = ((unsigned char)rand() % 28) + 1;
    unsigned char alpha = (unsigned char)(rand() % 26 + 'a');

    if (output_size + number > OUTPUT_SIZE) {
      number = (OUTPUT_SIZE - output_size);
    }
    *data_ptr = number;
    data_ptr++;
    *data_ptr = alpha;
...
void user_simulation_function() {
  ...
  int fd = vopen("csim_phy_file", O_WRONLY);
  unsigned long long write_bytes = 0;
  while (write_bytes != input_size) {
    write_bytes += vwrite(fd, input_data + write_bytes, input_size - write_bytes);
    std::cout << "vwrite()..." << std::endl;
    std::cout << "write_bytes = " << write_bytes << std::endl;
  }
  std::cout << "before vclose()..." << std::endl;
  vclose(fd);
  std::cout << "after vclose()..." << std::endl;
  ...
  for (int i = 0; i < OUTPUT_SIZE; i++) {
    if (expected_result[i] != real_file_buf[i]) {
      std::cout << "Mismatch at i = " << i
                ...
                << ", real = " << (int)real_file_buf[i] << std::endl;
      // exit(-1);
    }
  }
