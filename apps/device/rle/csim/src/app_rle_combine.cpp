...
void app_rle_combine(ST_Queue<bool> &reset_app_rle_combine,
...
#pragma HLS array_partition variable = data_tmp complete dim = 0
  ...
  while (1) {
    bool dummy;
    if (reset || (reset = reset_app_rle_combine.read_nb(dummy))) {
      remain_data_num = 0;
      last_write = false;
      struct APP_Data dummy0;
      app_rle_combine_input_data.read_nb(dummy0);
      reset_cnt++;
