...
#pragma HLS array_partition variable = results complete dim = 0
  ...
  while (1) {
    bool dummy;
    if (reset || (reset = reset_app_conv_mult_4.read_nb(dummy))) {
...
#pragma HLS pipeline
      mult_core(app_conv_mult_4_param, app_conv_mult_5_param,
                app_conv_mult_4_input_data, app_conv_mult_5_input_data,
                app_conv_row_reduce_4_input_data, kernels, &reg_input_data,
