...
void app_conv_mult_15( // total 8 mult engines
...
#pragma HLS array_partition variable = results complete dim = 0
  ...
  while (1) {
    bool dummy;
    if (reset || (reset = reset_app_conv_mult_15.read_nb(dummy))) {
...
#pragma HLS pipeline
      mult_core(app_conv_mult_15_param, app_conv_mult_dummy_param,
                app_conv_mult_15_input_data, app_conv_mult_dummy_input_data,
                app_conv_row_reduce_15_input_data, kernels, &reg_input_data,
