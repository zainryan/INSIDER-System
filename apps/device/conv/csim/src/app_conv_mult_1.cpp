...
#pragma HLS array_partition variable = results complete dim = 0
  ...
  while (1) {
    bool dummy;
    if (reset || (reset = reset_app_conv_mult_1.read_nb(dummy))) {
...
#pragma HLS pipeline
      mult_core(app_conv_mult_1_param, app_conv_mult_2_param,
                app_conv_mult_1_input_data, app_conv_mult_2_input_data,
                app_conv_row_reduce_1_input_data, kernels, &reg_input_data,
