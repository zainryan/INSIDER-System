...
void app_conv_mult_7( // total 8 mult engines
...
#pragma HLS array_partition variable = results complete dim = 0
  ...
  while (1) {
    bool dummy;
    if (reset || (reset = reset_app_conv_mult_7.read_nb(dummy))) {
...
#pragma HLS pipeline
      mult_core(app_conv_mult_7_param, app_conv_mult_8_param,
                app_conv_mult_7_input_data, app_conv_mult_8_input_data,
                app_conv_row_reduce_7_input_data, kernels, &reg_input_data,
