...
#pragma HLS array_partition variable = results complete dim = 0
  ...
  while (1) {
    bool dummy;
    if (reset || (reset = reset_app_conv_mult_9.read_nb(dummy))) {
...
#pragma HLS pipeline
      mult_core(app_conv_mult_9_param, app_conv_mult_10_param,
                app_conv_mult_9_input_data, app_conv_mult_10_input_data,
                app_conv_row_reduce_9_input_data, kernels, &reg_input_data,
