...
#pragma HLS array_partition variable = results complete dim = 0
  ...
  while (1) {
    bool dummy;
    if (reset || (reset = reset_app_conv_mult_12.read_nb(dummy))) {
...
#pragma HLS pipeline
      mult_core(app_conv_mult_12_param, app_conv_mult_13_param,
                app_conv_mult_12_input_data, app_conv_mult_13_input_data,
                app_conv_row_reduce_12_input_data, kernels, &reg_input_data,
